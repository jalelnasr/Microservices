...
const RecruiterOffres = () => {
  ...
  const createMutation = useMutation(offreApi.createOffre, {
    onSuccess: () => {
      queryClient.invalidateQueries('recruiter-offres');
            ...
            <Card>
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  <Typography variant="h6">{offre.titre}</Typography>
                  <Chip
                    label={offre.statut}
                ...
                <Chip label={offre.typeContrat} size="small" sx={{ mr: 1 }} />
                {offre.salaire && (
                  <Chip
                    label={`${offre.salaire.toLocaleString()} € / an`}
                    size="small"
                    color="primary"
                  />
                )}
...
export default RecruiterOffres;
